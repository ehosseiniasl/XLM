...
class TrainerAdpSpn(object):
    ...
    def __init__(self, data, params):
        ...
        device = self.model.in_emb.weight.device
        # initialize hidden cashe
        self.hid_cache = [
            torch.zeros(
                params.batch_size,
    ...
    def print_stats(self):
        ...
        self.writer.add_scalars(f'clm_en', {'train_clm_en_ppl': math.exp(np.mean(self.stats['CLM-en']))},
    ...
    def clm_step(self, lang1, lang2, lambda_coeff):
        ...
        input = x[:-1, :].long()
        target = x[1:, :].long()
        loss, self.hid_cache, _ = self.model(input.transpose(1,0), target.transpose(1,0), self.hid_cache)
...
class SingleTrainerAdpSpn(TrainerAdpSpn):
...
class EncDecTrainerAdpSpn(TrainerAdpSpn):
