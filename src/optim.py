...
class AdagradGCInverseSqrtWithWarmup(AdagradWithGradClip):
    """
    Decay the LR based on the inverse square root of the update number.
    We also support a warmup phase where we linearly increase the learning rate
    from some initial learning rate (`warmup-init-lr`) until the configured
...
class AdagradInverseSqrtWithWarmup(Adagrad):
    """
    Decay the LR based on the inverse square root of the update number.
    We also support a warmup phase where we linearly increase the learning rate
    from some initial learning rate (`warmup-init-lr`) until the configured
...
def get_optimizer(parameters, s, params=None):
    ...
    elif method == 'adagradgc_inverse_sqrt':
        optim_fn = AdagradGCInverseSqrtWithWarmup
