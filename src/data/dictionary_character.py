...
class DictionaryCharacter(object):
    ...
    def check_valid(self):
        ...
        # assert all(self.id2word[i] == SPECIAL_WORD % i for i in range(SPECIAL_WORDS))
        ...
        # for i in range(len(self.id2word) - 1):
    ...
    def min_count(self, min_count):
        ...
        #                 self.counts[self.id2word[k]] >= min_count}
        self.word2id = {v: k for k, v in self.id2word.items()}
        self.counts = {k: v for k, v in self.counts.items() if k in self.word2id}
        self.check_valid()
    ...
    def read_vocab(vocab_path):
        ...
        for i, line in enumerate(f):
            ...
            # word2id[line[0]] = i - skipped  # shift because of extra words
            counts[line[0]] = int(line[1])
        f.close()
        id2word = {v: k for k, v in word2id.items()}
        dico = DictionaryCharacter(id2word, word2id, counts)
